...
where s.class_type ='OnlineClass' 
...
and EXISTS (
	...
	where sr.relation_type ='school' 
	and sr.relation_id in (?)	
)`
		argsSchedule = append(argsSchedule, cond.SchoolIDs.Strings)
	}
...
and EXISTS (
	...
	and sr.relation_id in (?)	
		...
		argsSchedule = append(argsSchedule, cond.ClassIDs.Strings)
	}

	sbSchedule := NewSqlBuilder(ctx, sqlSchedule, argsSchedule...)
	sbAssessmentOnlineClass := NewSqlBuilder(ctx, `
	select id from assessments_v2 av  where av.assessment_type='OnlineClass' and av.schedule_id in ({{.sbSchedule}}) 
`).Replace(ctx, "sbSchedule", sbSchedule)
	sbAssessmentStudy := NewSqlBuilder(ctx, `
	select id from assessments_v2 av  where av.assessment_type in ('OnlineStudy','OfflineStudy') and av.schedule_id in ({{.sbSchedule}}) 
`).Replace(ctx, "sbSchedule", sbSchedule)
	sql := `
select
...
group by user_id 
`
	sb := NewSqlBuilder(ctx, sql).Replace(ctx, "sbAssessmentOnlineClass", sbAssessmentOnlineClass).
