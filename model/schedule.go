...
func (s *scheduleModel) VerifyLessonPlanAuthed(ctx context.Context, operator *entity.Operator, lessonPlanID string) (bool, error) {
	// verify lessPlan is valid
	contentMap, err := GetContentModel().GetContentsAuthProperty(ctx, []string{lessonPlanID}, operator)
	if err != nil {
		log.Error(ctx, "GetAuthedContentRecordsModel.GetContentsAuthProperty error", log.Err(err),
			log.String("lessonPlanID", lessonPlanID),
			log.Any("operator", operator),
		)
		return false, err
