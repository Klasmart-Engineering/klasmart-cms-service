...
SELECT cms_contents.id,
       substring_index(substring_index(cms_contents.outcomes, ',', b.help_topic_id + 1 ), ',',- 1) outcome_id
FROM cms_contents
         JOIN mysql.help_topic b ON b.help_topic_id < (length( cms_contents.outcomes )-length( REPLACE (cms_contents.outcomes, ',', '')) + 1)
