...
--                 select new_dir_path;
update cms_folder_items set dir_path=replace(dir_path, wdir_path, new_dir_path) where dir_path like concat('%', wid, '%') COLLATE utf8mb4_unicode_ci;
update cms_folder_items set dir_path= new_dir_path where id=wid COLLATE utf8mb4_unicode_ci;
end if;
end loop;
close cur;
end$$
