...
create procedure repaire_lost_folders()
    ...
    select dir_path into dist_path from cms_folder_items where id=wparent_id COLLATE utf8mb4_unicode_ci;
    set new_dir_path = concat(dist_path, '/', wparent_id);
    --                 select new_dir_path;
    update cms_folder_items set dir_path=replace(dir_path, wdir_path, new_dir_path), extra=concat(extra, if(extra is null or extra='', '', ';'), 'repair_folder_0901') where dir_path like concat('%', wid, '%') COLLATE utf8mb4_unicode_ci;
    update cms_folder_items set dir_path=new_dir_path, extra=concat(extra, if(extra is null or extra='', '', ';'), 'repair_folder_0901') where id=wid COLLATE utf8mb4_unicode_ci;
    end if;
    end loop;
    close cur;
end$$
