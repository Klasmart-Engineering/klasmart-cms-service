...
query ($or:[UserFilter!]){
	...
	for _, teacher := range data.Edges {
		...
		for _, class := range teacher.Node.ClassesTeachingConnection.Edges {
			if class.Node.Status != Active {
				continue
			}
			...
			for _, student := range class.Node.StudentsConnection.Edges {
				...
				classStudents.Students = append(classStudents.Students, StudentInClass{student.Node.ID})
			}
			tcs.ClassesTeaching = append(tcs.ClassesTeaching, classStudents)
		}
